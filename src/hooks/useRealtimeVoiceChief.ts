...
export const useRealtimeVoiceChief = () => {
  ...
  const processAudio = async (audioBlob: Blob) => {
    try {
      ...
      const transcribeResult = await supabase.functions.invoke(
        "realtime-voice-chief",
        {
          body: {
            action: "transcribe",
            audio: base64Audio,
      ...
      const chatResult = await supabase.functions.invoke(
        "realtime-voice-chief",
        {
          body: {
            action: "chat",
      ...
      const ttsResult = await supabase.functions.invoke(
        "realtime-voice-chief",
        {
          body: {
            action: "speak",
      ...
      try {
        ...
        if (cleanBase64.length % 4 !== 0) {
          console.error(
            "❌ Invalid base64 format - length not divisible by 4:",
            cleanBase64.length
          );
        ...
        try {
          ...
          console.log(
            "✅ Base64 decoded successfully, audio buffer size:",
            audioBuffer.length
          );
        } catch (decodeError) {
          console.error("❌ Base64 decode failed:", decodeError);
          console.error(
            "Base64 sample (100 chars):",
            cleanBase64.substring(0, 100)
          );
          throw new Error("Failed to decode base64 audio data");
        }

        const responseAudioBlob = new Blob([audioBuffer], {
          type: "audio/mpeg",
        });
        ...
        console.error(
          "Base64 audio response length:",
          base64AudioResponse?.length
        );
        console.error(
          "Base64 audio response sample:",
  ...
  const disconnect = useCallback(() => {
    ...
    // Removed toast notification for session ended
    // toast({
    //   title: "Disconnected",
    ...
    async (text: string) => {
      ...
      try {
        ...
        try {
          ...
          // Clean the base64 string (remove any whitespace or invalid characters)
          const cleanBase64 = base64AudioResponse.replace(
            /[^A-Za-z0-9+/=]/g,
            ""
          );
          ...
          const audioBuffer = Uint8Array.from(atob(cleanBase64), (c) =>
            c.charCodeAt(0)
          );
          const responseAudioBlob = new Blob([audioBuffer], {
            type: "audio/mpeg",
