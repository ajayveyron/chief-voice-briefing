...
export const ChiefInterface: React.FC<ChiefInterfaceProps> = ({ 
  ...
  const getActionIcon = (type: string) => {
    switch (type) {
      case 'send_email': return <Send className="h-3 w-3" />;
      ...
      <div className="p-4 border-b border-border bg-card">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-lg font-semibold text-foreground">Chief Assistant</h1>
              <p className="text-sm text-muted-foreground">
                AI Executive Assistant • {isVoiceMode ? 'Voice Mode' : 'Text Mode'}
              </p>
            </div>
          </div>
                ...
                >
                  ...
                  {message.actionSuggestions && message.actionSuggestions.length > 0 && (
                    <div className="mt-3 pt-3 border-t border-border/20">
                      <p className="text-xs font-medium mb-2 opacity-80">Suggested Actions:</p>
                      <div className="space-y-2">
                        {message.actionSuggestions.map((suggestion) => (
                          <Card key={suggestion.id} className="border-border/30">
                            <CardContent className="p-3">
                              <div className="flex items-center justify-between">
                                ...
                                <Badge variant={suggestion.requiresConfirmation ? "outline" : "secondary"}>
                                  {suggestion.requiresConfirmation ? "Manual" : "Auto"}
                                </Badge>
                              </div>
                  ...
                  {message.executionResults && message.executionResults.length > 0 && (
                    <div className="mt-3 pt-3 border-t border-border/20">
                      <p className="text-xs font-medium mb-2 opacity-80">Actions Executed:</p>
                      <div className="space-y-1">
                        {message.executionResults.map((result, index) => (
                          <div key={index} className="flex items-center space-x-2">
                            <CheckCircle className="h-3 w-3 text-emerald-500" />
                            <span className="text-xs">{result.message || 'Action completed'}</span>
                          </div>
                        ))}
                      </div>
                    </div>
        ...
        <div className="p-4 border-t border-border bg-card">
          <div className="mb-2">
            <p className="text-xs text-muted-foreground">
              Try: "Send an email to john@example.com about the meeting" or "What are my updates?"
            </p>
          </div>
