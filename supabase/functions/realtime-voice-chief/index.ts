...
serve(async (req) => {
  ...
  try {
    ...
    // Handle AI chat response with MCP tool integration
    if (action === "chat" && text) {
      ...
      if (
        (isEmailRequest || isCalendarRequest || isSlackRequest) &&
        authHeader
      ) {
        try {
          ...
          if (isEmailRequest) {
            ...
            const emailResponse = await fetch(
              `${supabaseUrl}/functions/v1/fetch-gmail-emails`,
              {
                method: "POST",
                headers: {
            ...
            if (emailResponse.ok) {
              ...
              if (emails.length === 0) {
                aiResponseText =
                  "You have no new emails to review right now. Your inbox is all caught up!";
              } else {
                aiResponseText = `You have ${emails.length} recent email${
                  emails.length !== 1 ? "s" : ""
                }:\n\n`;
                emails.slice(0, 3).forEach((email: any, index: number) => {
                  aiResponseText += `${index + 1}. From: ${email.from
                    .split("<")[0]
                ...
                aiResponseText +=
                  "Would you like me to read any specific email or help you respond to any of them?";
              }
            } else {
              aiResponseText =
                "I couldn't fetch your emails right now. Please make sure your Gmail is connected in the integrations.";
            }
          } else if (isCalendarRequest) {
            console.log("📅 Fetching calendar events...");
            const calendarResponse = await fetch(
              `${supabaseUrl}/functions/v1/fetch-calendar-events`,
              {
                method: "POST",
                headers: {
                  Authorization: authHeader,
            ...
            if (calendarResponse.ok) {
              ...
              if (events.length === 0) {
                aiResponseText =
                  "Your calendar is clear today. No meetings or events scheduled.";
              } else {
                aiResponseText = `You have ${events.length} upcoming event${
                  events.length !== 1 ? "s" : ""
                }:\n\n`;
              ...
              aiResponseText =
                "I couldn't fetch your calendar right now. Please make sure your calendar is connected in the integrations.";
            }
          } else if (isSlackRequest) {
            console.log("💬 Fetching Slack messages...");
            const slackResponse = await fetch(
              `${supabaseUrl}/functions/v1/fetch-slack-messages`,
              {
                method: "POST",
                headers: {
                  Authorization: authHeader,
            ...
            if (slackResponse.ok) {
              ...
              if (messages.length === 0) {
                ...
                aiResponseText = `You have ${
                  messages.length
                } recent Slack message${messages.length !== 1 ? "s" : ""}:\n\n`;
                messages.slice(0, 3).forEach((message: any, index: number) => {
                  aiResponseText += `${index + 1}. From ${
                    message.user
              ...
              aiResponseText =
                "I couldn't fetch your Slack messages right now. Please make sure Slack is connected in the integrations.";
            }
          }
        } catch (error) {
          console.error("Error fetching data:", error);
          aiResponseText =
            "I encountered an error while fetching your data. Please try again or check your integrations.";
        }
      } else {
        // Enhanced AI chat response with MCP tool awareness
...
MCP TOOLS: You have access to various tools through MCP servers. If a user requests an action that could be performed by a tool, you can suggest using available tools. Common tool categories include:
...
REMINDER: ENGLISH ONLY - NO EXCEPTIONS.`,
                ...
                {
                  role: "user",
                  content: `User said: "${text}". Please respond ONLY in English, regardless of the language the user used.`,
                },
              ],
              max_tokens: 500,
              temperature: 0.8,
        ...
        if (hasToolIntent) {
          ...
          aiResponseText +=
            "\n\nI can help you with various tasks using my available tools. Would you like me to perform any specific actions for you?";
        }
      }
    ...
    if (action === "speak" && text) {
      ...
      console.log(
        "✅ Audio generated successfully, base64 length:",
        base64Audio.length
      );
